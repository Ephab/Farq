...
HERMES_MODEL = os.getenv("HERMES_MODEL", "gemini-2.5-flash")
...
def run_agent(local_run_id: str, student_id: str) -> None:
    db = SessionLocal()
    ...
    try:
        if len(HERMES_API_KEY) < 16:
            raise RuntimeError("HERMES_API_KEY is missing or too short; run scripts/setup.ps1")
        run.status = "running"
        ...
        headers = {
            ...
            "X-Hermes-Session-Key": f"farq:user:{student_id}",
        }
        payload = {
            "input": (
